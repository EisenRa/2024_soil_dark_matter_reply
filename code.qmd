---
title: "Re-analysis of soil replies with SMF values"
...
OK, so the assertion that 4-9% of the base pairs are eukaryotic is a severe underestimate! `r scales::number((100 - mean(df$read_fraction)) / 4, accuracy = 0.1)`- to `r scales::number((100 - mean(df$read_fraction)) / 9, accuracy = 0.1)` -fold understimations to be precise.
...
1.  Accounts for the fact that different metagenomes can have different proportions of non-bacterial DNA
...
Despite their inaccurate AGS estimates, we still see a strong negative correlation as observed by Piton et al. Correlation = `r correlation`.
