...
AGS_overestimate_fold <- as.numeric(scales::number(6.8 / SMF_AGS, accuracy = 0.01)) AGS_overestimate_pc <- (1 - as.numeric(scales::number(SMF_AGS / 6.8, accuracy = 0.001))) * 100 
```
...
fig1 <- fig1a / (fig1b + fig1c) + plot_annotation(tag_levels = 'A')  ggsave("figures/figure_1.png", fig1, width = 9, height = 12)
```
